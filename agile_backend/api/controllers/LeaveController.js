...
module.exports = {
    async create(req, res) {
        try {
            ...
            /* status 1 -  success, status : 0 - record not created */
            const { userId, fromDate, toDate, allocatedLeave, leaveStatus, location, reason } = req.body;
            var Date1 = new Date(fromDate);
            var Date2 = new Date(toDate);

            // if (Date2 <= Date1) {
            //     return res.status(200).json({ status: 0, message: 'Please provide the valid start and end date.' });
            // }
            /** check the leave already exist **/
            const leaveRemaining = await LeaveRepo.checkLeaveApplied(req.body);
            console.log(leaveRemaining,'leaveRemaining');
            if (leaveRemaining.length) {
                return res.status(200).json({ status: 0, message: 'The leave is already applied.' });
                ...
                const leave = await LeaveRepo.create({ userId, fromDate, toDate, reasonforLeave: reason, leaveDescription: reason, allocatedLeave, remainingLeave: remaining, takenLeave: Days, leaveStatus, location, leaveType: reason });
                if (leave) {
                    return res.status(200).json({ status: 1, data: leave, message: 'Leave Created Successfully' });
                }
            }
        } catch (error) {
    ...
    async getAllList(location) {
        try {
            const list = await LeaveRepo.getAllList(location);
            if (list) {
                return list;
            }
