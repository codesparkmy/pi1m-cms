...
module.exports = {
    ...
    async getOne(req, res) {
        try {
            ...
            if (!!user) {
                const cafeDetails = await UserRepo.getCafeDetails(user.branchAllocation);
                if (!!cafeDetails) {
                    user.cafelocation = cafeDetails.location;
                    user.cafeBranch = cafeDetails.branchName;
                }
    ...
    async rolePermissionscreateAndUpdate(req, res) {
        const subpage = await UserRepo.modifyPermissions(req.body);
        console.log(subpage, "subpage");
        return res.status(200).json({ subpage });
    },
    ...
    //   async topup(req,res){
    //     console.log("topup");
    //     console.log(req.body,"topup-body");
    //     const user = await UserRepo.getOne(userId);
    // },
    ...
    async resetPassword(req, res) {
        console.log(req.params, "body-update");
        try {
        ...
        try {
            ...
            if (!!result) {
                ...
                var mailSend = transporter.sendMail({
                    ...
                    html: `<html><body><div><span>Hi ${result.fullName}</span><p>Please click here and reset your password</p><a href="${sails.config.custom.baseUrl}/webe/#/resetPassword/:${result.id}">Click Here</a><div></body></html>`
                });
        ...
        uploadFile.upload({ dirname: require('path').resolve(sails.config.appPath, 'assets/images') }, async function onUploadComplete(err, files) {
            ...
            if (!!isUpdated) {
                var data = {
                    ...
                    'sails.config.appPath': sails.config.appPath,
                }
                return res.status(200).json({ message: 'Successfully uploaded file', data: data });
            } else {
                return res.notFound();
            }
