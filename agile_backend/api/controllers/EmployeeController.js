...
module.exports = {
    async create(req, res) {
        try {
            ...
            console.log(branchAllocation)
                // const user = await UserRepo.create({ userstatus:true, email, phoneNumber,fullName,gender, dob });
            ...
            if (user === undefined || user == null) {
                ...
                const result = await EmployeeRepo.create({ fullName, email, phoneNumber, gender, dob, employeeId, subscriptionId, RaceId, ReligionCId, jobCategory, positionId, joiningDate, citizenShip, nationality, businessFax, branchAllocation, password: encPassword, profilePicture, role });
                if (result) {
                    ...
                    //   html: `<html><body><div><span>Hi ${fullName}</span><p>Your temporary password ${password}</p><div></body></html>`
                    // });    
    ...
    async updateCronEmpDetails(request) {
        try {
            const listvalidate = await EmployeeRepo.empRecords(request.email, request.branchAllocation);
            if(listvalidate) {
                var executedRecord = await EmployeeRepo.empRecordsUpdate(request);
                if(!!executedRecord) {
                    return "success";
                } else {
                    return "failed";
                ...
                var executedRecord = await EmployeeRepo.empRecordsCreate(request);
                if(!!executedRecord) {
                    return "success";
                } else {
                    return "failed";
