...
module.exports = {
    ...
    async updateHqDetails(req, res) {
        try {
            const list = await leavetypeRepo.findhqcmsRecord(req.body);
            if(list) {
                const list = await leavetypeRepo.findhqcmsupdate(req.body);
                return res.status(200).json({ data: list });        

            } else {
                const list = await leavetypeRepo.findhqcmscreate(req.body);
                return res.status(200).json({ data: list });        
            }
