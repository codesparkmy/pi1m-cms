...
module.exports = {
    ...
    async updateHqDetails(req, res) {
        try {
            const list = await IncomeLevelRepo.findhqcmsRecord(req.body);
            if(list) {
                const list = await IncomeLevelRepo.findhqcmsupdate(req.body);
                return res.status(200).json({ data: list });        

            } else {
                const list = await IncomeLevelRepo.findhqcmscreate(req.body);
                return res.status(200).json({ data: list });        
            }
