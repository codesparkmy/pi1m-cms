...
module.exports = {
   ...
   async updateHqDetails(req, res) {
        try {
            const list = await trainingCategoryRepo.findhqcmsRecord(req.body);
            if(list) {
                const list = await trainingCategoryRepo.findhqcmsupdate(req.body);
                return res.status(200).json({ data: list });        

            } else {
                const list = await trainingCategoryRepo.findhqcmscreate(req.body);
                return res.status(200).json({ data: list });        
            }
