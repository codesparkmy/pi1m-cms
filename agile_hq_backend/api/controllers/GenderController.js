...
module.exports = {
    ...
    async updateHqDetails(req, res) {
        try {
            const list = await genderRepo.findhqcmsRecord(req.body);
            if(list) {
                const list = await genderRepo.findhqcmsupdate(req.body);
                return res.status(200).json({ data: list });        

            } else {
                const list = await genderRepo.findhqcmscreate(req.body);
                return res.status(200).json({ data: list });        
            }
