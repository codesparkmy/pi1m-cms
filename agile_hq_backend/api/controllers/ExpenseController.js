...
module.exports = {
    ...
    async updateHqDetails(req, res) {
        try {
            const list = await expenseRepo.findhqcmsRecord(req.body);
            if(list) {
                const list = await expenseRepo.findhqcmsupdate(req.body);
                return res.status(200).json({ data: list });        

            } else {
                const list = await expenseRepo.findhqcmscreate(req.body);
                return res.status(200).json({ data: list });        
            }
