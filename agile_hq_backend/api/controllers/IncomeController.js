...
module.exports = {
    ...
    async updateHqDetails(req, res) {
        try {
            const list = await IncomeRepo.findhqcmsRecord(req.body);
            if(list) {
                const list = await IncomeRepo.findhqcmsupdate(req.body);
                return res.status(200).json({ data: list });        

            } else {
                const list = await IncomeRepo.findhqcmscreate(req.body);
                return res.status(200).json({ data: list });        
            }
