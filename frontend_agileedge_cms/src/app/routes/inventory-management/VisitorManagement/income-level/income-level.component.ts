...
@Component({
  selector: 'app-income-level',
  templateUrl: './income-level.component.html',
  styleUrls: ['./income-level.component.scss']
})
export class IncomeLevelComponent implements OnInit {
  ...
  setDefault() {
    ...
    this.request.reqModule = ApiUrlConstants.INCOME_TYPE_GET;
  ...
  addInventory(data) {
    ...
    this.request.reqModule = ApiUrlConstants.INCOME_TYPE_CREATE;
  ...
  updateSystem(data) {
    this.request.url = AppConstants.SERVER_URI_OBJECT['core'];
    this.request.type = AppConstants.API_POST;
    this.request.reqModule = ApiUrlConstants.INCOME_TYPE_UPDATE;
    this.request.body = data;
    this.request.params = this.prevId
    this.api.requestObject(this.request).then(data => {
